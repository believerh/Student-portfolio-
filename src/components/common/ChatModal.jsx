...
const ChatModal = ({
  ...
  useKeyboardClose(showChat, () => setShowChat(false));
  const chatContainerRef = useFocusTrap(showChat);
  ...
  return (
    <div
      ...
      className="fixed inset-0 bg-gradient-to-br from-indigo-900/80 via-purple-900/80 to-pink-900/80 flex items-center justify-center z-50 p-2 sm:p-4 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
        ...
        <div className={`p-2 sm:p-3 border-b ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'} flex-shrink-0`}>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
            <button
              ...
              aria-label={`Select chat recipient: ${getRecipientLabel()}`}
              aria-expanded={showRecipientSelect}
            >
        ...
        {showRecipientSelect && (
          <div className={`p-2 sm:p-4 border-b ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'} flex-shrink-0`}>
            ...
            <button 
              ...
              aria-label="Cancel recipient selection"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-2 sm:space-y-4">
          {filteredMessages.length === 0 ? (
            ...
            filteredMessages.map((msg) => {
              const isMyMessage = msg.sender_id === (currentUser?.dbId || currentUser?.id);
