...
import { FileText, Image, Video, Music, Eye, Trash2, Share2, MessageSquare, Heart, Send, X, Loader2, Tag, CircleHelp, Download, RefreshCw, ImagePlus, Video as VideoIcon, Music as MusicIcon, FileText as FileIcon, PlayCircle } from 'lucide-react';
...
const FileViewer = memo(function FileViewer(props) {
  ...
  useEffect(() => {
    const loadAIData = async () => {
      ...
      try {
        // Load tags
        const tagsResponse = await fetch(`/rest/v1/file_tags?file_id=eq.${file.id}&select=tag`, {
          headers: {
            'apikey': dbConfig?.key || '',
        ...
        if (tagsResponse.ok) {
          ...
          const tags = tagsData.map((t: any) => t.tag);
          setFileTags(tags);
        }
        
        // Load summary
        const summaryResponse = await fetch(`/rest/v1/file_summaries?file_id=eq.${file.id}&select=summary`, {
          headers: {
            'apikey': dbConfig?.key || '',
            'Authorization': `Bearer ${dbConfig?.key || ''}`
          }
        ...
        const analyticsResponse = await fetch(`/rest/v1/file_analytics?file_id=eq.${file.id}&select=*`, {
          headers: {
            'apikey': dbConfig?.key || '',
            'Authorization': `Bearer ${dbConfig?.key || ''}`
          }
        ...
        if (analyticsResponse.ok) {
          ...
          if (analyticsData.length > 0) {
            ...
            await fetch(`/rest/v1/file_analytics?id=eq.${analyticsRecord.id}`, {
              method: 'PATCH',
              headers: {
                'apikey': dbConfig?.key || '',
                'Authorization': `Bearer ${dbConfig?.key || ''}`,
            ...
            await fetch(`/rest/v1/file_analytics`, {
              method: 'POST',
              headers: {
                'apikey': dbConfig?.key || '',
                'Authorization': `Bearer ${dbConfig?.key || ''}`,
        ...
        // Continue without AI/analytics data
      }
    };
    ...
    <div className={`border-2 ${darkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-100'} rounded-xl overflow-hidden hover:border-indigo-300 hover:shadow-md transition-all duration-300`}>
           <div className="flex items-center justify-between p-2 sm:p-4 gap-2">
             <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
              ...
              {/* AI Summary Section (shown on hover or click) */}
              {fileSummary && (
                <div className="mt-1">
                  <div 
          ...
          <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
               {showActions && (
                 <>
                   ...
                   {/* Refresh Preview Button (for media files) */}
                   {(file.type === 'video' || file.type === 'image' || file.type === 'audio') && (
                     <button
                       onClick={() => {
                         setMediaLoaded(false);
                         setMediaLoading(true);
                         // Force reload by changing key or src
                         showNotification('Refreshing preview...');
                       }}
                       className={`flex items-center gap-1 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg ${darkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'} transition-all`}
                     >
                 ...
                 <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} p-3 sm:p-6 rounded-lg max-h-48 sm:max-h-96 overflow-auto`}>
                   <div className="mb-2">
                     <div className="flex items-center justify-between mb-2">
                       ...
                       <span className="text-xs">{Math.floor(file.size / 1024)} KB</span>
                     </div>
                   </div>
                   <iframe 
                     src={file.data} 
